...
subroutine compute_true_passFlag2( &
  ...
  ! -------------------------------------------------------------------------
  ! Step 1: Round + scale "real" scenario
  ! -------------------------------------------------------------------------
  do j = 1, ncols
     do i = 1, nrows
        if (mask(i,j) == 1) then
...
! -------------------------------------------------------------------------
! Step 5: Rule 2 pass/fail logic
!
!   Condition 1: total < 24 => fail
!   Condition 2,3 removed (no HL/SL checks)
...
! -------------------------------------------------------------------------
  ...
  ! Condition 1
  if (observed_student_totals(i) < 24.0d0) then
  ...
  ! If any fail reason triggered => pass=0
  ...
  deallocate(temp, cnt, tk_ee_count)
  return
end subroutine compute_true_passFlag2


subroutine add_error_scale_total_and_assign2( &
     data, diagVar, scaleFactor, mask, group, nrows, ncols, nGroups, &
     nBound, nBoundMax, bound_ub, grade_val, &
  ...
  ! If any condition triggered => pass=0
  if (sum(failReasonsObs(i,:)) > 0) then
  passFlag(i) = 0
  end if
  end do

  ! Step 6: Compare real vs obs
  do i=1,nrows
     do k=1,7
  ...
  deallocate(temp,totals,finalGrades,cnt,student_totals,hl_totals,sl_totals, &
             passFlag,failCounter,tk_ee_count,grade_23_count)
  return
end subroutine add_error_scale_total_and_assign2

subroutine check_seed_size2(n, ierr)
...
end subroutine check_seed_size2

subroutine init_fortran_seed2(ierr)
  ...
  deallocate(seed)
end subroutine init_fortran_seed2