subroutine add_error_scale_total_and_assign_retake1( &
...
do i = 1, nrows
  ...
  ! --- Condition 2: HL < 9 => fail
  if (hl_totals_retake(i) < 9.0d0) then
    observed_passFlag_nov(i) = 0
  end if
...
! Condition 2: HL < 9 => fail
if (hl_totals_retake_error(i) < 9.0d0) then
observed_passFlag_nov_error(i) = 0
end if
...
deallocate(reasonDiff, score100, scoreNov, retake_probability, retake_decision, failReasonsObs,  &
  ...
  return
  end subroutine add_error_scale_total_and_assign_retake1

  subroutine init_fortran_seed_retake1(ierr)
  implicit none
  integer, intent(out) :: ierr
  integer :: n, i
  ...
  deallocate(seed)
  return
  end subroutine init_fortran_seed_retake1
