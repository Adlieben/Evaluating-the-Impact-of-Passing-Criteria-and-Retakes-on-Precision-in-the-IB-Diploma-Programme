subroutine add_error_scale_total_and_assign_retake( &
...
do i = 1, nrows
  ...
  if (student_totals_retake(i) < 24.0d0) then
    observed_passFlag_nov(i) = 0
  end if

  ! --- Condition 2: HL < 12 => fail
  if (hl_totals_retake(i) < 12.0d0) then
    observed_passFlag_nov(i) = 0
  end if

  ! --- Condition 3: SL < 9 => fail
  if (sl_totals_retake(i) < 9.0d0) then
    observed_passFlag_nov(i) = 0
  end if
...
do i = 1, nrows
  ...
  ! Condition 1: total < 24 => fail
  if (student_totals_retake_error(i) < 24.0d0) then
    observed_passFlag_nov_error(i) = 0
  end if

  ! Condition 2: HL < 12 => fail
  if (hl_totals_retake_error(i) < 12.0d0) then
    observed_passFlag_nov_error(i) = 0
  end if

  ! Condition 3: SL < 9 => fail
  if (sl_totals_retake_error(i) < 9.0d0) then
    observed_passFlag_nov_error(i) = 0
  end if
...
deallocate(reasonDiff, score100, scoreNov, retake_probability, retake_decision, failReasonsObs,  &
  ...
  return
  end subroutine add_error_scale_total_and_assign_retake

  subroutine init_fortran_seed_retake(ierr)
  implicit none
  integer, intent(out) :: ierr
  integer :: n, i
  ...
  deallocate(seed)
  return
  end subroutine init_fortran_seed_retake
