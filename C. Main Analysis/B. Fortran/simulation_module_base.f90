subroutine add_error_scale_total_and_assign( &
     ...
     failReasonsObs,    & 
     reasonDiff,        &
     sensitivity, specificity, ppv, npv, accuracy, reasonMatrix,      &
     ierr)
  ...
  ! New: real fail reasons so we can compare
  ...
  integer, intent(out)              :: reasonMatrix(7,7)
  ...
  do i = 1, nrows
     ...
     ! Condition 1
     if (student_totals(i) < 24.0d0) then
        failReasonsObs(i,1) = 1
     end if

     ! Condition 2
     if (hl_totals(i) < 12.0d0) then
        failReasonsObs(i,2) = 1
     end if

     ! Condition 3
     if (sl_totals(i) < 9.0d0) then
  ...
  ! -------------------------------------------------------------------------
  ! Step 6: See differences in fail reasons between real and observed
  ! -------------------------------------------------------------------------
  ...
  do i = 1, nrows
     ...
     ! 6A) The "single-dimension" difference count
     do k = 1, 7
     ...
     ! 6B) The crossed count of differences in fail reasons triggered
     do k1 = 1, 7
        if (failReasonsReal(i,k1) /= failReasonsObs(i,k1)) then
           do k2 = 1, 7
              if (failReasonsReal(i,k2) /= failReasonsObs(i,k2)) then
                 reasonMatrix(k1, k2) = reasonMatrix(k1, k2) + 1
              end if
           end do
        end if
     end do
  ...
  deallocate(temp, totals, finalGrades, cnt, student_totals, hl_totals, sl_totals, &
             passFlag, failCounter, tk_ee_count, grade_23_count)
  return
end subroutine add_error_scale_total_and_assign
      
subroutine compute_true_passFlag( &
    data, diagVar, scaleFactor, mask, group, nrows, ncols, nGroups, &
    nBound, nBoundMax, bound_ub, grade_val, &
  ...
  ! Step 6: Assign real pass/fail + track which condition(s) caused fail
  do i = 1, nrows
     do k = 1, 7
        failReasonsReal(i,k) = 0
     end do
     observed_passFlag(i) = 1  ! default pass

     ! Condition 1
     if (observed_student_totals(i) < 24d0) then
        failReasonsReal(i,1) = 1
     end if
     ! Condition 2
     if (observed_hl_totals(i) < 12d0) then
        failReasonsReal(i,2) = 1
     end if
     ! Condition 3
     if (observed_sl_totals(i) < 9d0) then
        failReasonsReal(i,3) = 1
     end if
     ! Condition 4
     do g = 1, nGroups
        if (finalGrades(i,g) == 1d0) then
  ...
  ! -------------------------------------------------------------------------
  ! NEW: Copy local arrays to user outputs
  ! -------------------------------------------------------------------------
  ! 1) componentScoresReal = final scaled component values in temp
  do i = 1, nrows
     do j = 1, ncols
  ...
  ! 2) groupTotalsReal = the "totals" array
  do i = 1, nrows
     do g = 1, nGroups
  ...
  ! 3) subjectGradesReal = finalGrades array
  do i = 1, nrows
     do g = 1, nGroups
  ...
  return
end subroutine compute_true_passFlag

subroutine check_seed_size(n, ierr)
...
end subroutine check_seed_size

subroutine init_fortran_seed(ierr)
  ...
  deallocate(seed)
end subroutine init_fortran_seed
