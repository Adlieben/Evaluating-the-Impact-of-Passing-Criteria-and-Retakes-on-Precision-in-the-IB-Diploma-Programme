subroutine add_error_scale_total_and_assign_retake5( &
...
do i = 1, nrows
 ...
 ! Condition 1
 if (student_totals(i) < 18.0d0) then
...
do i = 1, nrows
  ...
  if (student_totals_retake(i) < 18.0d0) then
  ...
  ! --- Condition 6: count how many 2’s
  grade2count  = 0
  grade23count = 0
  do g = 1, nGroups
    if (tk_ee_flag(g) /= 1) then
...
do i = 1, nrows
  ...
  ! Condition 1: total < 18 => fail
  if (student_totals_retake_error(i) < 18.0d0) then
...
deallocate(reasonDiff, score100, scoreNov, retake_probability, retake_decision, failReasonsObs,  &
  ...
  return
  end subroutine add_error_scale_total_and_assign_retake5

  subroutine init_fortran_seed_retake5(ierr)
  implicit none
  integer, intent(out) :: ierr
  integer :: n, i
  ...
  deallocate(seed)
  return
  end subroutine init_fortran_seed_retake5
