! -------------------------------------------------------------------------------------
! simulation_module_rule3.f90
! Implements "Rule 3":
!   - No HL/SL checks (like Rule 1).
!   - Condition 1 => total < 20 (changed from <24 in the baseline).
! -------------------------------------------------------------------------------------

subroutine compute_true_passFlag3( &
  ...
  ! -------------------------------------------------------------------------
  ! Step 0: Initialization and memory allocation
  ! -------------------------------------------------------------------------
  ierr_local = 0
  ...
  ! -------------------------------------------------------------------------
  ! Step 1: Round observed data + apply scale factors (no random error here)
  ! -------------------------------------------------------------------------
  do j = 1, ncols
     do i = 1, nrows
        if (mask(i,j) == 1) then
  ...
  ! Round totals or mark -999 if no valid cells
  do i = 1, nrows
     do g = 1, nGroups
        if (cnt(i,g) == 0) then
           totals(i,g) = -999.d0
  ...
  do i = 1, nrows
     ...
     ! If both TK and EE found, apply possible bonus
  ...
  ! -------------------------------------------------------------------------
  ! Step 4.1: For completeness, compute HL and SL totals
  ! -------------------------------------------------------------------------
  do i = 1, nrows
  ...
  ! -------------------------------------------------------------------------
  ! Step 5: Rule 3 pass/fail logic
  !   Condition 1 => total < 20 => fail
  !   Condition 2,3 => removed (no HL or SL check)
  !   Condition 4 => any grade=1 => fail
  ...
  do i = 1, nrows
     ...
     ! Condition 1 => if total < 20
     if (observed_student_totals(i) < 20.d0) then
  ...
  deallocate(temp, cnt, tk_ee_count)
  return
end subroutine compute_true_passFlag3


subroutine add_error_scale_total_and_assign3( &
     data, diagVar, scaleFactor, mask, group, nrows, ncols, nGroups, &
     nBound, nBoundMax, bound_ub, grade_val, &
  ...
  ! Step 5: pass/fail logic for Rule 3
  !  Condition 1 => total<20
  ...
  do i=1,nrows
     ...
     ! Condition 1 => total<20
     if (student_totals(i)<20.d0) then
  ...
  ! Step 6: Compare failReasonsObs vs failReasonsReal => reasonDiff
  do i=1,nrows
     do k=1,7
  ...
  deallocate(temp, totals, finalGrades, cnt, student_totals, hl_totals, sl_totals, &
             passFlag, failCounter, tk_ee_count, grade_23_count)
  return
end subroutine add_error_scale_total_and_assign3

subroutine check_seed_size3(n, ierr)
...
end subroutine check_seed_size3

subroutine init_fortran_seed3(ierr)
  ...
  deallocate(seed)
end subroutine init_fortran_seed3